...
plot_mean_base <- function(mean_score, ordIndex)
{
...
plot_bayes_bw <- function(bayes_score, ordIndex, bw_result, bw_ord)
{
  if (length(unique(bw_result)) == 3)
  {
    G1.id = which(rr == 1)
    S.id = which(rr == 2)
    G2M.id = which(rr == 3)
    ...
    cell = bayes_score[ordIndex, ][bw_ord, ]
...
plot_mean_bw <- function(mean_score, ordIndex, bw_result, bw_ord)
{
  if (length(unique(bw_result)) == 3)
  {
    G1.id = which(rr == 1)
    ...
    cell = mean_score[ordIndex, ][bw_ord, ]
